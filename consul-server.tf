...
resource "null_resource" "consul_server_start" {

  for_each = local.consul_servers_map

  depends_on = [
    null_resource.consul_server_setup,
  ]

  triggers = {
    setup_id = null_resource.consul_server_setup[each.key].id
  }
    ...
    content = templatefile("${path.module}/scripts/set-consul-envs.sh.tpl",
      {
        ...
        server_index      = each.value.local_index
    })
  }
