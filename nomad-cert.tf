...
resource "tls_cert_request" "nomad_cli" {
  ...
  private_key_pem = tls_private_key.nomad_cli[each.key].private_key_pem

  subject {
    common_name = "cli.${each.value.nomad_region}.nomad"
  }

  dns_names = [
    "cli.${each.value.nomad_region}.nomad",
    "localhost",
    "127.0.0.1"
  ]
}
...
resource "tls_locally_signed_cert" "nomad_cli" {
  ...
  cert_request_pem   = tls_cert_request.nomad_cli[each.key].cert_request_pem
  ca_private_key_pem = local.cert_ssh_private_key
  ca_cert_pem        = tls_self_signed_cert.nomad_ca.cert_pem
